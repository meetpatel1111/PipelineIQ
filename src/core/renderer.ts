import type { FailureEvent, JiraTicketSpec } from "./types/index.js";
import type { ComputedMetrics } from "./types/index.js";
import { maskSecrets } from "./secret-mask.js";
import { buildSmartExcerpt } from "./log-parser/smart-excerpt.js";

/**
 * Human-friendly exit code explanations
 */
export function getExitCodeDescription(code?: number): string | null {
  if (code === undefined || code === null) return null;
  switch (code) {
    case 0: return "0 (Success)";
    case 1: return "1 (General Command / Script Failure)";
    case 2: return "2 (Misuse of shell builtins / Syntax Error)";
    case 126: return "126 (Command cannot execute / Permission Denied)";
    case 127: return "127 (Command Not Found in PATH)";
    case 128: return "128 (Invalid exit argument)";
    case 130: return "130 (Terminated by Ctrl+C / SIGINT)";
    case 137: return "137 (Killed by OS - Out of Memory / SIGKILL)";
    case 139: return "139 (Segmentation Fault / SIGSEGV)";
    case 143: return "143 (Terminated by SIGTERM)";
    default: return `${code}`;
  }
}

/**
 * Canonical alias lookup table for metadata field keys.
 * Maps user-friendly shorthand, snake_case, and kebab-case aliases
 * to their internal allFields key.
 */
export const METADATA_FIELD_ALIASES: Record<string, string> = {
  // Repository
  repo: "repository",
  reponame: "repoName",
  repo_name: "repoName",
  "repo-name": "repoName",
  repository_name: "repoName",
  "repository-name": "repoName",
  project_name: "repoName",
  // Commit
  sha: "commit",
  commit_id: "commit",
  commitid: "commit",
  "commit-id": "commit",
  commit_sha: "commit",
  commitsha: "commit",
  "commit-sha": "commit",
  hash: "commit",
  git_commit: "commit",
  // Run Number
  run_number: "runNumber",
  runnumber: "runNumber",
  "run-number": "runNumber",
  run_id: "runNumber",
  runid: "runNumber",
  "run-id": "runNumber",
  build_number: "runNumber",
  buildnumber: "runNumber",
  "build-number": "runNumber",
  build_id: "runNumber",
  buildid: "runNumber",
  pipeline_number: "runNumber",
  // Pipeline / Workflow
  pipeline_name: "pipeline",
  workflow: "pipeline",
  workflow_name: "pipeline",
  job: "job",
  job_name: "jobName",
  // URLs
  url: "runUrl",
  run_url: "runUrl",
  runurl: "runUrl",
  "run-url": "runUrl",
  build_url: "runUrl",
  buildurl: "runUrl",
  // Steps & Stages
  step_name: "step",
  failed_step: "step",
  "failed-step": "step",
  stage_name: "stage",
  failed_stage: "stage",
  "failed-stage": "stage",
  // Exit code
  exit_code: "exitCode",
  exitcode: "exitCode",
  "exit-code": "exitCode",
  // Triggered by / Actor
  actor: "triggeredBy",
  author: "triggeredBy",
  user: "triggeredBy",
  triggered_by: "triggeredBy",
  "triggered-by": "triggeredBy",
  // Branch & PR
  branch_name: "branch",
  pr: "pullRequest",
  pr_number: "pullRequest",
  pull_request: "pullRequest",
  // Duration
  duration_ms: "duration",
  elapsed: "duration",
  runtime: "duration",
};

/**
 * Build the markdown ticket description from the event + already-populated fields.
 * Called late in the pipeline, after all enrichers have run.
 */
export function renderDescription(
  event: FailureEvent,
  fields: Partial<JiraTicketSpec>,
  logExcerptLines: number,
  maskLogs: boolean,
  displayMetadata?: string[],
  history?: {
    similarCount: number;
    isFlaky: boolean;
    flakinessScore?: number | undefined;
    retryResolvedCount?: number | undefined;
    previousIncidentKeys: string[];
    trend?: "improving" | "worsening" | "stable" | undefined;
    relatedKeys: string[];
  },
  metrics?: ComputedMetrics,
): string {
  const out: string[] = [];

  out.push("## PipelineIQ Failure Report");
  out.push("");

  // Failure summary section
  const rca = fields.rca;
  const remediation = fields.remediationSteps;

  out.push("### Failure Summary");
  let summary = fields.summary ?? "Pipeline failure detected.";
  if (fields.customFields?.["_matchConfidence"] !== undefined) {
    const conf = Math.round(Number(fields.customFields["_matchConfidence"]) * 100);
    summary += ` (Match Confidence: ${conf}%)`;
  }
  out.push(summary);
  out.push("");

  // Incident Breakdown (Rich Structural Details Without AI)
  const step = event.pipeline.step ?? event.failure.failedStep;
  const exitDesc = getExitCodeDescription(event.failure.exitCode);
  if (step || exitDesc || fields.category) {
    out.push("### Incident Breakdown");
    out.push(`- **Pipeline:** \`${event.pipeline.name}\``);
    if (step) out.push(`- **Failing Step:** \`${step}\``);
    if (event.pipeline.stage) out.push(`- **Failing Stage:** \`${event.pipeline.stage}\``);
    if (exitDesc) out.push(`- **Exit Status:** \`${exitDesc}\``);
    if (fields.category && fields.category !== "Unknown") out.push(`- **Classification:** \`${fields.category}\``);
    if (event.failure.errorMessage) {
      const firstLine = event.failure.errorMessage.trim().split("\n")[0]!;
      out.push(`- **Key Diagnostic:** \`${firstLine.length > 100 ? firstLine.substring(0, 97) + "..." : firstLine}\``);
    }
    out.push("");
  }

  if (rca) {
    out.push("### Root Cause");
    out.push(rca);
    out.push("");
  }

  if (remediation && Array.isArray(remediation) && remediation.length > 0) {
    out.push("### Suggested Remediation");
    remediation.forEach((step, i) => out.push(`${i + 1}. ${step}`));
    out.push("");
  }

  out.push("---");
  out.push("");

  // Reliability Context (Historical)
  if (history) {
    out.push("### Reliability Context");
    
    let trendIcon = "➡️";
    if (history.trend === "worsening") trendIcon = "📈";
    if (history.trend === "improving") trendIcon = "📉";

    const flakyMsg = history.isFlaky
      ? ` ⚠️ **Detected as Flaky**${history.flakinessScore !== undefined && history.flakinessScore > 0 ? ` (${Math.round(history.flakinessScore * 100)}% flakiness score)` : ""}`
      : "";
    
    out.push(`- **Frequency:** ${history.similarCount} occurrences in last 30 days${flakyMsg}`);
    if (history.trend) {
      out.push(`- **Trend:** ${trendIcon} ${history.trend.charAt(0).toUpperCase() + history.trend.slice(1)}`);
    }
    
    if (history.previousIncidentKeys.length > 0) {
      const keys = history.previousIncidentKeys.slice(0, 5).join(", ");
      const more = history.previousIncidentKeys.length > 5 ? "..." : "";
      out.push(`- **Previous Incidents:** ${keys}${more}`);
    }

    if (history.relatedKeys.length > 0) {
      out.push(`- **Related by Symptom:** ${history.relatedKeys.join(", ")}`);
    }

    if (metrics) {
      if (metrics.mttrHours !== undefined && metrics.sampleSize > 0) {
        out.push(`- **MTTR:** ${metrics.mttrHours}h avg (${metrics.sampleSize} incidents)`);
      }
      if (metrics.blastRadius !== undefined) {
        out.push(`- **Blast radius:** ${metrics.blastRadius} repos affected`);
      }
    }
    out.push("");
    out.push("---");
    out.push("");
  }

  const commitMsg = event.commit.message ?? "";
  const firstLine = commitMsg.split("\n")[0] || "";
  const truncatedMsg = firstLine.substring(0, 80) + (commitMsg.length > 80 ? "..." : "");

  // Define all available fields for the metadata table
  const allFields = [
    { key: "source", label: "Source", value: event.source },
    {
      key: "pipeline",
      label: "Pipeline",
      value: event.pipeline.url ? `[${event.pipeline.name}](${event.pipeline.url})` : event.pipeline.name,
    },
    {
      key: "runUrl",
      label: "Pipeline Run",
      value: event.pipeline.runUrl ? `[View Execution](${event.pipeline.runUrl})` : "",
    },
    {
      key: "repository",
      label: "Repository",
      value: event.repository.url
        ? `[${event.repository.owner}/${event.repository.name}](${event.repository.url})`
        : `${event.repository.owner}/${event.repository.name}`,
    },
    {
      key: "repoName",
      label: "Repository Name",
      value: event.repository.name,
    },
    {
      key: "pullRequest",
      label: "Pull Request",
      value: event.pullRequest
        ? (event.pullRequest.url ? `[#${event.pullRequest.number} ${event.pullRequest.title}](${event.pullRequest.url})` : `#${event.pullRequest.number} ${event.pullRequest.title}`)
        : "",
    },
    { key: "branch", label: "Branch", value: event.branch },
    {
      key: "commit",
      label: "Commit",
      value: event.commit.url
        ? `[\`${event.commit.sha.slice(0, 7)}\`](${event.commit.url})`
        : `\`${event.commit.sha.slice(0, 7)}\``,
    },
    {
      key: "commitMessage",
      label: "Commit Message",
      value: truncatedMsg,
    },
    { key: "environment", label: "Environment", value: event.environment ?? "" },
    { key: "step", label: "Failed Step", value: event.pipeline.step ?? "" },
    { key: "stage", label: "Failed Stage", value: event.pipeline.stage ?? "" },
    { key: "exitCode", label: "Exit Code", value: event.failure.exitCode?.toString() ?? "" },
    {
      key: "retryCount",
      label: "Retry Count",
      value: event.pipeline.retryCount?.toString() ?? "",
    },
    {
      key: "runAttempt",
      label: "Run Attempt",
      value: event.pipeline.runAttempt?.toString() ?? "",
    },
    { key: "job", label: "Job", value: event.pipeline.job ?? "" },
    { key: "jobName", label: "Job Name", value: event.pipeline.jobName ?? "" },
    { key: "eventName", label: "Event Name", value: event.eventName ?? "" },
    { key: "runnerOs", label: "Runner OS", value: event.pipeline.runnerOs ?? "" },
    { key: "runnerArch", label: "Runner Arch", value: event.pipeline.runnerArch ?? "" },
    { key: "runnerType", label: "Runner Type", value: event.pipeline.runnerType ?? "" },
    { key: "runnerEnvironment", label: "Runner Environment", value: event.pipeline.runnerEnvironment ?? "" },
    { key: "runnerDebug", label: "Runner Debug", value: event.pipeline.runnerDebug !== undefined ? String(event.pipeline.runnerDebug) : "" },
    { key: "runNumber", label: "Run Number", value: event.pipeline.runNumber?.toString() ?? "" },
    { key: "triggeredBy", label: "Triggered By", value: event.triggeredBy ?? "unknown" },
    { key: "refType", label: "Ref Type", value: event.pipeline.refType ?? "" },
    { key: "workflowRef", label: "Workflow Ref", value: event.pipeline.workflowRef ?? "" },
    { key: "reason", label: "Build Reason", value: event.pipeline.reason ?? "" },
    { key: "teamProject", label: "Team Project", value: event.pipeline.teamProject ?? "" },
    { key: "teamProjectId", label: "Team Project ID", value: event.pipeline.teamProjectId ?? "" },
    { key: "agentPool", label: "Agent Pool", value: event.pipeline.agentPool ?? "" },
    { key: "buildUri", label: "Build URI", value: event.pipeline.buildUri ?? "" },
    { key: "buildNumber", label: "Build Number", value: event.pipeline.buildNumber ?? "" },
    { key: "workflowSha", label: "Workflow SHA", value: event.pipeline.workflowSha ?? "" },
    { key: "action", label: "Action", value: event.pipeline.action ?? "" },
    { key: "actionPath", label: "Action Path", value: event.pipeline.actionPath ?? "" },
    { key: "actionRepository", label: "Action Repo", value: event.pipeline.actionRepository ?? "" },
    { key: "baseRef", label: "Base Ref", value: event.pipeline.baseRef ?? "" },
    { key: "headRef", label: "Head Ref", value: event.pipeline.headRef ?? "" },
    { key: "runnerTemp", label: "Runner Temp", value: event.pipeline.runnerTemp ?? "" },
    { key: "runnerToolCache", label: "Runner Tool Cache", value: event.pipeline.runnerToolCache ?? "" },
    { key: "runnerWorkspace", label: "Runner Workspace", value: event.pipeline.runnerWorkspace ?? "" },
    { key: "workspace", label: "Workspace", value: event.pipeline.workspace ?? "" },
    { key: "jobStatus", label: "Job Status", value: event.pipeline.jobStatus ?? "" },
    { key: "jobContainer", label: "Job Container", value: event.pipeline.jobContainer ?? "" },
    { key: "jobServices", label: "Job Services", value: event.pipeline.jobServices ?? "" },
    { key: "strategyIndex", label: "Matrix Index", value: event.pipeline.strategyJobIndex?.toString() ?? "" },
    { key: "strategyTotal", label: "Matrix Total", value: event.pipeline.strategyJobTotal?.toString() ?? "" },
    { key: "actionRef", label: "Action Ref", value: event.pipeline.actionRef ?? "" },
    { key: "actionStatus", label: "Action Status", value: event.pipeline.actionStatus ?? "" },
    { key: "repositoryGitUrl", label: "Repo Git URL", value: event.pipeline.repositoryGitUrl ?? "" },
    { key: "repositoryClean", label: "Repo Clean", value: event.pipeline.repositoryClean ?? "" },
    { key: "repositoryGitSubmoduleCheckout", label: "Git Submodule Checkout", value: event.pipeline.repositoryGitSubmoduleCheckout ?? "" },
    { key: "checksStageAttempt", label: "Checks Stage Attempt", value: event.pipeline.checksStageAttempt ?? "" },
    { key: "strategyName", label: "Strategy Name", value: event.pipeline.strategyName ?? "" },
    { key: "strategyCycleName", label: "Strategy Cycle Name", value: event.pipeline.strategyCycleName ?? "" },
    { key: "cronScheduleDisplayName", label: "Cron Display Name", value: event.pipeline.cronScheduleDisplayName ?? "" },
    { key: "secretSource", label: "Secret Source", value: event.pipeline.secretSource ?? "" },
    { key: "eventPayload", label: "Event Payload", value: event.eventPayload ? "Included (JSON)" : "" },
    { key: "retentionDays", label: "Log Retention", value: event.pipeline.retentionDays?.toString() ?? "" },
    { key: "refProtected", label: "Ref Protected", value: event.pipeline.refProtected !== undefined ? String(event.pipeline.refProtected) : "" },
    { key: "apiUrl", label: "API URL", value: event.apiUrl ?? "" },
    { key: "graphqlUrl", label: "GraphQL URL", value: event.graphqlUrl ?? "" },
    { key: "agentContainerMapping", label: "Container Mapping", value: event.pipeline.agentContainerMapping ?? "" },
    { key: "agentReleaseDirectory", label: "Release Dir", value: event.pipeline.agentReleaseDirectory ?? "" },
    { key: "agentRootDirectory", label: "Agent Root", value: event.pipeline.agentRootDirectory ?? "" },
    { key: "pipelineWorkspace", label: "Pipeline Workspace", value: event.pipeline.pipelineWorkspace ?? "" },
    { key: "systemDebug", label: "System Debug", value: event.pipeline.systemDebug ?? "" },
    { key: "systemDefaultWorkingDirectory", label: "Default Working Dir", value: event.pipeline.systemDefaultWorkingDirectory ?? "" },
    { key: "systemCollectionUri", label: "Collection URI", value: event.pipeline.systemCollectionUri ?? "" },
    { key: "systemTeamFoundationCollectionUri", label: "TF Collection URI", value: event.pipeline.systemTeamFoundationCollectionUri ?? "" },
    { key: "releaseDeploymentRequestedFor", label: "Release Requested For", value: event.pipeline.releaseDeploymentRequestedFor ?? "" },
    { key: "releaseDeploymentRequestedForEmail", label: "Release Requester Email", value: event.pipeline.releaseDeploymentRequestedForEmail ?? "" },
    { key: "releaseDeploymentId", label: "Release Deployment ID", value: event.pipeline.releaseDeploymentId ?? "" },
    { key: "releaseDefinitionEnvironmentId", label: "Release Def Env ID", value: event.pipeline.releaseDefinitionEnvironmentId ?? "" },
    { key: "releaseDefinitionId", label: "Release Def ID", value: event.pipeline.releaseDefinitionId ?? "" },
    { key: "releaseDefinitionName", label: "Release Def Name", value: event.pipeline.releaseDefinitionName ?? "" },
    { key: "releaseEnvironmentId", label: "Release Env ID", value: event.pipeline.releaseEnvironmentId ?? "" },
    { key: "releaseEnvironmentName", label: "Release Env Name", value: event.pipeline.releaseEnvironmentName ?? "" },
    { key: "releasePrimaryArtifactSourceAlias", label: "Primary Artifact Alias", value: event.pipeline.releasePrimaryArtifactSourceAlias ?? "" },
    { key: "releaseDescription", label: "Release Description", value: event.pipeline.releaseDescription ?? "" },
    { key: "releaseId", label: "Release ID", value: event.pipeline.releaseId ?? "" },
    { key: "releaseName", label: "Release Name", value: event.pipeline.releaseName ?? "" },
    { key: "releaseUri", label: "Release URI", value: event.pipeline.releaseUri ?? "" },
    { key: "systemJobDisplayName", label: "Job Display Name", value: event.pipeline.systemJobDisplayName ?? "" },
    { key: "systemJobId", label: "Job ID", value: event.pipeline.systemJobId ?? "" },
    { key: "systemJobName", label: "System Job Name", value: event.pipeline.systemJobName ?? "" },
    { key: "systemPhaseAttempt", label: "Phase Attempt", value: event.pipeline.systemPhaseAttempt ?? "" },
    { key: "systemStageAttempt", label: "Stage Attempt", value: event.pipeline.systemStageAttempt ?? "" },
    { key: "systemStageDisplayName", label: "Stage Display Name", value: event.pipeline.systemStageDisplayName ?? "" },
    { key: "systemStageName", label: "Stage Name", value: event.pipeline.systemStageName ?? "" },
    { key: "systemWorkFolder", label: "System Work Folder", value: event.pipeline.systemWorkFolder ?? "" },
    { key: "tfBuild", label: "TF Build", value: event.pipeline.tfBuild ?? "" },
    { key: "systemPhaseDisplayName", label: "Phase Display Name", value: event.pipeline.systemPhaseDisplayName ?? "" },
    { key: "systemPhaseName", label: "Phase Name", value: event.pipeline.systemPhaseName ?? "" },
    { key: "systemPlanId", label: "System Plan ID", value: event.pipeline.systemPlanId ?? "" },
    { key: "systemTimelineId", label: "System Timeline ID", value: event.pipeline.systemTimelineId ?? "" },
    { key: "systemCollectionId", label: "System Collection ID", value: event.pipeline.systemCollectionId ?? "" },
    { key: "systemHostType", label: "System Host Type", value: event.pipeline.systemHostType ?? "" },
    { key: "prIsFork", label: "PR Is Fork", value: event.pipeline.prIsFork ?? "" },
    { key: "prId", label: "PR ID", value: event.pipeline.prId ?? "" },
    { key: "prNumber", label: "PR Number", value: event.pipeline.prNumber ?? "" },
    { key: "prTargetBranchName", label: "PR Target Branch", value: event.pipeline.prTargetBranchName ?? "" },
    { key: "prSourceBranch", label: "PR Source Branch", value: event.pipeline.prSourceBranch ?? "" },
    { key: "prSourceCommitId", label: "PR Source Commit", value: event.pipeline.prSourceCommitId ?? "" },
    { key: "prSourceRepoUri", label: "PR Source Repo URI", value: event.pipeline.prSourceRepoUri ?? "" },
    { key: "prTargetBranch", label: "PR Target Branch (full)", value: event.pipeline.prTargetBranch ?? "" },
    { key: "stageRequestedBy", label: "Stage Requested By", value: event.pipeline.stageRequestedBy ?? "" },
    { key: "stageRequestedForId", label: "Stage Requester ID", value: event.pipeline.stageRequestedForId ?? "" },
    { key: "triggeredByDefinitionName", label: "Triggered By Pipeline", value: event.pipeline.triggeredByDefinitionName ?? "" },
    { key: "triggeredByBuildNumber", label: "Triggered By Build #", value: event.pipeline.triggeredByBuildNumber ?? "" },
    { key: "triggeredByDefinitionId", label: "Triggered By Def ID", value: event.pipeline.triggeredByDefinitionId ?? "" },
    { key: "triggeredByBuildId", label: "Triggered By Build ID", value: event.pipeline.triggeredByBuildId ?? "" },
    { key: "environmentResourceName", label: "Env Resource Name", value: event.pipeline.environmentResourceName ?? "" },
    { key: "environmentId", label: "Environment ID", value: event.pipeline.environmentId ?? "" },
    { key: "sourceTfvcShelveset", label: "TFVC Shelveset", value: event.pipeline.sourceTfvcShelveset ?? "" },
    { key: "definitionId", label: "Definition ID", value: event.pipeline.definitionId ?? "" },
    { key: "agentId", label: "Agent ID", value: event.pipeline.agentId ?? "" },
    { key: "agentName", label: "Agent Name", value: event.pipeline.runnerName ?? "" },
    { key: "agentMachineName", label: "Agent Machine", value: event.pipeline.agentMachineName ?? "" },
    { key: "agentJobStatus", label: "Agent Job Status", value: event.pipeline.agentJobStatus ?? "" },
    { key: "agentBuildDirectory", label: "Agent Build Dir", value: event.pipeline.agentBuildDirectory ?? "" },
    { key: "agentHomeDirectory", label: "Agent Home Dir", value: event.pipeline.agentHomeDirectory ?? "" },
    { key: "agentTempDirectory", label: "Agent Temp Dir", value: event.pipeline.agentTempDirectory ?? "" },
    { key: "agentToolsDirectory", label: "Agent Tools Dir", value: event.pipeline.agentToolsDirectory ?? "" },
    { key: "agentWorkFolder", label: "Agent Work Folder", value: event.pipeline.agentWorkFolder ?? "" },
    { key: "artifactStagingDirectory", label: "Artifact Staging Dir", value: event.pipeline.artifactStagingDirectory ?? "" },
    { key: "binariesDirectory", label: "Binaries Dir", value: event.pipeline.binariesDirectory ?? "" },
    { key: "containerId", label: "Container ID", value: event.pipeline.containerId ?? "" },
    { key: "definitionVersion", label: "Definition Version", value: event.pipeline.definitionVersion ?? "" },
    { key: "repositoryLocalPath", label: "Repo Local Path", value: event.pipeline.repositoryLocalPath ?? "" },
    { key: "sourcesDirectory", label: "Sources Dir", value: event.pipeline.sourcesDirectory ?? "" },
    { key: "stagingDirectory", label: "Staging Dir", value: event.pipeline.stagingDirectory ?? "" },
    { key: "testResultsDirectory", label: "Test Results Dir", value: event.pipeline.testResultsDirectory ?? "" },
    { key: "requestedFor", label: "Requested For", value: event.pipeline.requestedFor ?? "" },
    { key: "requestedForEmail", label: "Requester Email", value: event.pipeline.requestedForEmail ?? "" },
    { key: "requestedForId", label: "Requester ID", value: event.pipeline.requestedForId ?? "" },
    { key: "queuedBy", label: "Queued By", value: event.pipeline.queuedBy ?? "" },
    { key: "queuedById", label: "Queued By ID", value: event.pipeline.queuedById ?? "" },
    { key: "sourceBranchName", label: "Source Branch Name", value: event.pipeline.sourceBranchName ?? "" },
    { key: "fullSourceBranch", label: "Source Branch (full ref)", value: event.pipeline.fullSourceBranch ?? "" },
    { key: "sourceVersionMessage", label: "Commit Message", value: event.pipeline.sourceVersionMessage ?? "" },
    { key: "repositoryId", label: "Repository ID", value: event.pipeline.repositoryId ?? "" },
    { key: "repositoryProvider", label: "Repo Provider", value: event.pipeline.repositoryProvider ?? "" },
    { key: "repositoryUri", label: "Repo URI", value: event.pipeline.repositoryUri ?? "" },
    // GitLab CI metadata rows
    { key: "gitlabProjectId", label: "GitLab Project ID", value: event.pipeline.gitlabProjectId ?? "" },
    { key: "gitlabProjectUrl", label: "GitLab Project URL", value: event.pipeline.gitlabProjectUrl ?? "" },
    { key: "gitlabProjectTitle", label: "GitLab Project Title", value: event.pipeline.gitlabProjectTitle ?? "" },
    { key: "gitlabProjectDescription", label: "GitLab Project Description", value: event.pipeline.gitlabProjectDescription ?? "" },
    { key: "gitlabProjectVisibility", label: "GitLab Visibility", value: event.pipeline.gitlabProjectVisibility ?? "" },
    { key: "gitlabProjectNamespace", label: "GitLab Namespace", value: event.pipeline.gitlabProjectNamespace ?? "" },
    { key: "gitlabProjectRootNamespace", label: "GitLab Root Namespace", value: event.pipeline.gitlabProjectRootNamespace ?? "" },
    { key: "gitlabPipelineId", label: "GitLab Pipeline ID", value: event.pipeline.gitlabPipelineId ?? "" },
    { key: "gitlabPipelineIid", label: "GitLab Pipeline IID", value: event.pipeline.gitlabPipelineIid ?? "" },
    { key: "gitlabPipelineUrl", label: "GitLab Pipeline URL", value: event.pipeline.gitlabPipelineUrl ?? "" },
    { key: "gitlabPipelineSource", label: "GitLab Pipeline Source", value: event.pipeline.gitlabPipelineSource ?? "" },
    { key: "gitlabPipelineName", label: "GitLab Pipeline Name", value: event.pipeline.gitlabPipelineName ?? "" },
    { key: "gitlabPipelineCreatedAt", label: "GitLab Pipeline Created At", value: event.pipeline.gitlabPipelineCreatedAt ?? "" },
    { key: "gitlabJobId", label: "GitLab Job ID", value: event.pipeline.gitlabJobId ?? "" },
    { key: "gitlabJobName", label: "GitLab Job Name", value: event.pipeline.gitlabJobName ?? "" },
    { key: "gitlabJobStage", label: "GitLab Job Stage", value: event.pipeline.gitlabJobStage ?? "" },
    { key: "gitlabJobStatus", label: "GitLab Job Status", value: event.pipeline.gitlabJobStatus ?? "" },
    { key: "gitlabJobStartedAt", label: "GitLab Job Started At", value: event.pipeline.gitlabJobStartedAt ?? "" },
    { key: "gitlabJobImage", label: "GitLab Job Image", value: event.pipeline.gitlabJobImage ?? "" },
    { key: "gitlabJobTimeout", label: "GitLab Job Timeout", value: event.pipeline.gitlabJobTimeout ?? "" },
    { key: "gitlabJobTags", label: "GitLab Job Tags", value: event.pipeline.gitlabJobTags ?? "" },
    { key: "gitlabCommitTitle", label: "GitLab Commit Title", value: event.pipeline.gitlabCommitTitle ?? "" },
    { key: "gitlabCommitDescription", label: "GitLab Commit Description", value: event.pipeline.gitlabCommitDescription ?? "" },
    { key: "gitlabCommitAuthor", label: "GitLab Commit Author", value: event.pipeline.gitlabCommitAuthor ?? "" },
    { key: "gitlabCommitTimestamp", label: "GitLab Commit Timestamp", value: event.pipeline.gitlabCommitTimestamp ?? "" },
    { key: "gitlabCommitBeforeSha", label: "GitLab Before SHA", value: event.pipeline.gitlabCommitBeforeSha ?? "" },
    { key: "gitlabEnvironmentTier", label: "GitLab Environment Tier", value: event.pipeline.gitlabEnvironmentTier ?? "" },
    { key: "gitlabEnvironmentUrl", label: "GitLab Environment URL", value: event.pipeline.gitlabEnvironmentUrl ?? "" },
    { key: "gitlabEnvironmentAction", label: "GitLab Environment Action", value: event.pipeline.gitlabEnvironmentAction ?? "" },
    { key: "gitlabMergeRequestIid", label: "GitLab MR IID", value: event.pipeline.gitlabMergeRequestIid ?? "" },
    { key: "gitlabMergeRequestId", label: "GitLab MR ID", value: event.pipeline.gitlabMergeRequestId ?? "" },
    { key: "gitlabMergeRequestTitle", label: "GitLab MR Title", value: event.pipeline.gitlabMergeRequestTitle ?? "" },
    { key: "gitlabMergeRequestEvent", label: "GitLab MR Event", value: event.pipeline.gitlabMergeRequestEvent ?? "" },
    { key: "gitlabMergeRequestSourceBranch", label: "GitLab MR Source Branch", value: event.pipeline.gitlabMergeRequestSourceBranch ?? "" },
    { key: "gitlabMergeRequestTargetBranch", label: "GitLab MR Target Branch", value: event.pipeline.gitlabMergeRequestTargetBranch ?? "" },
    { key: "gitlabMergeRequestAssignees", label: "GitLab MR Assignees", value: event.pipeline.gitlabMergeRequestAssignees ?? "" },
    { key: "gitlabMergeRequestLabels", label: "GitLab MR Labels", value: event.pipeline.gitlabMergeRequestLabels ?? "" },
    { key: "gitlabMergeRequestMilestone", label: "GitLab MR Milestone", value: event.pipeline.gitlabMergeRequestMilestone ?? "" },
    { key: "gitlabRunnerId", label: "GitLab Runner ID", value: event.pipeline.gitlabRunnerId ?? "" },
    { key: "gitlabRunnerDescription", label: "GitLab Runner Desc", value: event.pipeline.gitlabRunnerDescription ?? "" },
    { key: "gitlabRunnerTags", label: "GitLab Runner Tags", value: event.pipeline.gitlabRunnerTags ?? "" },
    { key: "gitlabRunnerVersion", label: "GitLab Runner Version", value: event.pipeline.gitlabRunnerVersion ?? "" },
    { key: "gitlabUserLogin", label: "GitLab User Login", value: event.pipeline.gitlabUserLogin ?? "" },
    { key: "gitlabUserName", label: "GitLab User Name", value: event.pipeline.gitlabUserName ?? "" },
    { key: "gitlabUserEmail", label: "GitLab User Email", value: event.pipeline.gitlabUserEmail ?? "" },
    { key: "gitlabUserId", label: "GitLab User ID", value: event.pipeline.gitlabUserId ?? "" },
    { key: "gitlabServerUrl", label: "GitLab Server URL", value: event.pipeline.gitlabServerUrl ?? "" },
    { key: "gitlabServerVersion", label: "GitLab Server Version", value: event.pipeline.gitlabServerVersion ?? "" },
    // Bitbucket Pipelines metadata rows
    { key: "bitbucketBuildNumber", label: "Bitbucket Build Number", value: event.pipeline.bitbucketBuildNumber?.toString() ?? "" },
    { key: "bitbucketCloneDir", label: "Bitbucket Clone Dir", value: event.pipeline.bitbucketCloneDir ?? "" },
    { key: "bitbucketCommit", label: "Bitbucket Commit", value: event.pipeline.bitbucketCommit ?? "" },
    { key: "bitbucketWorkspace", label: "Bitbucket Workspace", value: event.pipeline.bitbucketWorkspace ?? "" },
    { key: "bitbucketWorkspaceUuid", label: "Bitbucket Workspace UUID", value: event.pipeline.bitbucketWorkspaceUuid ?? "" },
    { key: "bitbucketRepoOwner", label: "Bitbucket Repo Owner", value: event.pipeline.bitbucketRepoOwner ?? "" },
    { key: "bitbucketRepoOwnerUuid", label: "Bitbucket Repo Owner UUID", value: event.pipeline.bitbucketRepoOwnerUuid ?? "" },
    { key: "bitbucketRepoSlug", label: "Bitbucket Repo Slug", value: event.pipeline.bitbucketRepoSlug ?? "" },
    { key: "bitbucketRepoUuid", label: "Bitbucket Repo UUID", value: event.pipeline.bitbucketRepoUuid ?? "" },
    { key: "bitbucketRepoFullName", label: "Bitbucket Repo Full Name", value: event.pipeline.bitbucketRepoFullName ?? "" },
    { key: "bitbucketRepoIsPrivate", label: "Bitbucket Repo Is Private", value: event.pipeline.bitbucketRepoIsPrivate !== undefined ? String(event.pipeline.bitbucketRepoIsPrivate) : "" },
    { key: "bitbucketBranch", label: "Bitbucket Branch", value: event.pipeline.bitbucketBranch ?? "" },
    { key: "bitbucketTag", label: "Bitbucket Tag", value: event.pipeline.bitbucketTag ?? "" },
    { key: "bitbucketBookmark", label: "Bitbucket Bookmark", value: event.pipeline.bitbucketBookmark ?? "" },
    { key: "bitbucketParallelStep", label: "Bitbucket Parallel Step", value: event.pipeline.bitbucketParallelStep?.toString() ?? "" },
    { key: "bitbucketParallelStepCount", label: "Bitbucket Parallel Steps Total", value: event.pipeline.bitbucketParallelStepCount?.toString() ?? "" },
    { key: "bitbucketPrId", label: "Bitbucket PR ID", value: event.pipeline.bitbucketPrId?.toString() ?? "" },
    { key: "bitbucketPrDestinationBranch", label: "Bitbucket PR Dest Branch", value: event.pipeline.bitbucketPrDestinationBranch ?? "" },
    { key: "bitbucketPrDestinationCommit", label: "Bitbucket PR Dest Commit", value: event.pipeline.bitbucketPrDestinationCommit ?? "" },
    { key: "bitbucketMergeQueuePrIds", label: "Bitbucket Merge Queue PRs", value: event.pipeline.bitbucketMergeQueuePrIds ?? "" },
    { key: "bitbucketGitHttpOrigin", label: "Bitbucket Git HTTP Origin", value: event.pipeline.bitbucketGitHttpOrigin ?? "" },
    { key: "bitbucketGitSshOrigin", label: "Bitbucket Git SSH Origin", value: event.pipeline.bitbucketGitSshOrigin ?? "" },
    { key: "bitbucketExitCode", label: "Bitbucket Exit Code", value: event.pipeline.bitbucketExitCode?.toString() ?? "" },
    { key: "bitbucketStepUuid", label: "Bitbucket Step UUID", value: event.pipeline.bitbucketStepUuid ?? "" },
    { key: "bitbucketPipelineUuid", label: "Bitbucket Pipeline UUID", value: event.pipeline.bitbucketPipelineUuid ?? "" },
    { key: "bitbucketDeploymentEnvironment", label: "Bitbucket Deployment Env", value: event.pipeline.bitbucketDeploymentEnvironment ?? "" },
    { key: "bitbucketDeploymentEnvironmentUuid", label: "Bitbucket Deployment Env UUID", value: event.pipeline.bitbucketDeploymentEnvironmentUuid ?? "" },
    { key: "bitbucketProjectKey", label: "Bitbucket Project Key", value: event.pipeline.bitbucketProjectKey ?? "" },
    { key: "bitbucketProjectUuid", label: "Bitbucket Project UUID", value: event.pipeline.bitbucketProjectUuid ?? "" },
    { key: "bitbucketStepTriggererUuid", label: "Bitbucket Step Triggerer UUID", value: event.pipeline.bitbucketStepTriggererUuid ?? "" },
    { key: "bitbucketStepRunNumber", label: "Bitbucket Step Run Number", value: event.pipeline.bitbucketStepRunNumber?.toString() ?? "" },
    { key: "bitbucketPackagesUsername", label: "Bitbucket Packages Username", value: event.pipeline.bitbucketPackagesUsername ?? "" },
    { key: "bitbucketDockerHost", label: "Bitbucket Docker Host", value: event.pipeline.bitbucketDockerHost ?? "" },
    { key: "bitbucketTriggerPipelineUuid", label: "Bitbucket Trigger Pipeline UUID", value: event.pipeline.bitbucketTriggerPipelineUuid ?? "" },
    { key: "bitbucketTriggerPipelineRunUuid", label: "Bitbucket Trigger Run UUID", value: event.pipeline.bitbucketTriggerPipelineRunUuid ?? "" },
    { key: "bitbucketTriggerStepUuid", label: "Bitbucket Trigger Step UUID", value: event.pipeline.bitbucketTriggerStepUuid ?? "" },
    { key: "bitbucketTriggerPipelineSelectorType", label: "Bitbucket Trigger Selector Type", value: event.pipeline.bitbucketTriggerPipelineSelectorType ?? "" },
    { key: "bitbucketTriggerPipelineSelectorPattern", label: "Bitbucket Trigger Selector Pattern", value: event.pipeline.bitbucketTriggerPipelineSelectorPattern ?? "" },
    { key: "bitbucketTriggerPipelineStatus", label: "Bitbucket Trigger Pipeline Status", value: event.pipeline.bitbucketTriggerPipelineStatus ?? "" },
    { key: "bitbucketTriggerDeploymentUuid", label: "Bitbucket Trigger Deployment UUID", value: event.pipeline.bitbucketTriggerDeploymentUuid ?? "" },
    { key: "bitbucketTriggerDeploymentStatus", label: "Bitbucket Trigger Deployment Status", value: event.pipeline.bitbucketTriggerDeploymentStatus ?? "" },
    { key: "bitbucketTriggerDeploymentEnvironmentName", label: "Bitbucket Trigger Deployment Env Name", value: event.pipeline.bitbucketTriggerDeploymentEnvironmentName ?? "" },
    { key: "bitbucketTriggerPackagesPackageType", label: "Bitbucket Trigger Package Type", value: event.pipeline.bitbucketTriggerPackagesPackageType ?? "" },
    { key: "bitbucketTriggerPackagesPackageName", label: "Bitbucket Trigger Package Name", value: event.pipeline.bitbucketTriggerPackagesPackageName ?? "" },
    { key: "bitbucketTriggerPackagesArtifactName", label: "Bitbucket Trigger Artifact Name", value: event.pipeline.bitbucketTriggerPackagesArtifactName ?? "" },
    { key: "bitbucketTriggerFixFlakyTestTargetBranch", label: "Bitbucket Flaky Test Target Branch", value: event.pipeline.bitbucketTriggerFixFlakyTestTargetBranch ?? "" },
    { key: "bitbucketTriggerFixFlakyTestSourceBranch", label: "Bitbucket Flaky Test Source Branch", value: event.pipeline.bitbucketTriggerFixFlakyTestSourceBranch ?? "" },
    { key: "bitbucketTriggerTestCaseFqdn", label: "Bitbucket Test Case FQDN", value: event.pipeline.bitbucketTriggerTestCaseFqdn ?? "" },
    { key: "bitbucketTriggerTestCaseUuid", label: "Bitbucket Test Case UUID", value: event.pipeline.bitbucketTriggerTestCaseUuid ?? "" },
    // CircleCI fields
    { key: "circleciBuildNum", label: "CircleCI Job Number", value: event.pipeline.circleciBuildNum?.toString() ?? "" },
    { key: "circleciBuildUrl", label: "CircleCI Job URL", value: event.pipeline.circleciBuildUrl ?? "" },
    { key: "circleciJob", label: "CircleCI Job Name", value: event.pipeline.circleciJob ?? "" },
    { key: "circleciBranch", label: "CircleCI Branch", value: event.pipeline.circleciBranch ?? "" },
    { key: "circleciTag", label: "CircleCI Tag", value: event.pipeline.circleciTag ?? "" },
    { key: "circleciSha1", label: "CircleCI Commit SHA", value: event.pipeline.circleciSha1 ?? "" },
    { key: "circleciRepositoryUrl", label: "CircleCI Repository URL", value: event.pipeline.circleciRepositoryUrl ?? "" },
    { key: "circleciProjectReponame", label: "CircleCI Project Repo", value: event.pipeline.circleciProjectReponame ?? "" },
    { key: "circleciProjectUsername", label: "CircleCI Project Org/User", value: event.pipeline.circleciProjectUsername ?? "" },
    { key: "circleciProjectId", label: "CircleCI Project ID", value: event.pipeline.circleciProjectId ?? "" },
    { key: "circleciOrganizationId", label: "CircleCI Organization ID", value: event.pipeline.circleciOrganizationId ?? "" },
    { key: "circleciPipelineId", label: "CircleCI Pipeline ID", value: event.pipeline.circleciPipelineId ?? "" },
    { key: "circleciPipelineNumber", label: "CircleCI Pipeline Number", value: event.pipeline.circleciPipelineNumber?.toString() ?? "" },
    { key: "circleciWorkflowId", label: "CircleCI Workflow ID", value: event.pipeline.circleciWorkflowId ?? "" },
    { key: "circleciWorkflowJobId", label: "CircleCI Workflow Job ID", value: event.pipeline.circleciWorkflowJobId ?? "" },
    { key: "circleciWorkflowWorkspaceId", label: "CircleCI Workspace ID", value: event.pipeline.circleciWorkflowWorkspaceId ?? "" },
    { key: "circleciWorkingDirectory", label: "CircleCI Working Directory", value: event.pipeline.circleciWorkingDirectory ?? "" },
    { key: "circleciNodeIndex", label: "CircleCI Parallel Node Index", value: event.pipeline.circleciNodeIndex?.toString() ?? "" },
    { key: "circleciNodeTotal", label: "CircleCI Parallel Nodes Total", value: event.pipeline.circleciNodeTotal?.toString() ?? "" },
    { key: "circleciPrNumber", label: "CircleCI PR Number", value: event.pipeline.circleciPrNumber?.toString() ?? "" },
    { key: "circleciPrReponame", label: "CircleCI PR Repo", value: event.pipeline.circleciPrReponame ?? "" },
    { key: "circleciPrUsername", label: "CircleCI PR Author", value: event.pipeline.circleciPrUsername ?? "" },
    { key: "circleciPullRequest", label: "CircleCI Pull Request", value: event.pipeline.circleciPullRequest ?? "" },
    { key: "circleciPullRequests", label: "CircleCI Pull Requests", value: event.pipeline.circleciPullRequests ?? "" },
    { key: "circleciUsername", label: "CircleCI Triggering User", value: event.pipeline.circleciUsername ?? "" },
    // Jenkins fields
    { key: "jenkinsBuildNumber", label: "Jenkins Build Number", value: event.pipeline.jenkinsBuildNumber ?? "" },
    { key: "jenkinsBuildId", label: "Jenkins Build ID", value: event.pipeline.jenkinsBuildId ?? "" },
    { key: "jenkinsBuildDisplayName", label: "Jenkins Build Display Name", value: event.pipeline.jenkinsBuildDisplayName ?? "" },
    { key: "jenkinsBuildTag", label: "Jenkins Build Tag", value: event.pipeline.jenkinsBuildTag ?? "" },
    { key: "jenkinsJobName", label: "Jenkins Job Name", value: event.pipeline.jenkinsJobName ?? "" },
    { key: "jenkinsJobBaseName", label: "Jenkins Job Base Name", value: event.pipeline.jenkinsJobBaseName ?? "" },
    { key: "jenkinsBuildUrl", label: "Jenkins Build URL", value: event.pipeline.jenkinsBuildUrl ?? "" },
    { key: "jenkinsJobUrl", label: "Jenkins Job URL", value: event.pipeline.jenkinsJobUrl ?? "" },
    { key: "jenkinsUrl", label: "Jenkins Master URL", value: event.pipeline.jenkinsUrl ?? "" },
    { key: "jenkinsHome", label: "Jenkins Home", value: event.pipeline.jenkinsHome ?? "" },
    { key: "jenkinsNodeName", label: "Jenkins Node Name", value: event.pipeline.jenkinsNodeName ?? "" },
    { key: "jenkinsNodeLabels", label: "Jenkins Node Labels", value: event.pipeline.jenkinsNodeLabels ?? "" },
    { key: "jenkinsExecutorNumber", label: "Jenkins Executor Number", value: event.pipeline.jenkinsExecutorNumber ?? "" },
    { key: "jenkinsWorkspace", label: "Jenkins Workspace", value: event.pipeline.jenkinsWorkspace ?? "" },
    { key: "jenkinsWorkspaceTmp", label: "Jenkins Workspace Tmp", value: event.pipeline.jenkinsWorkspaceTmp ?? "" },
    { key: "jenkinsBranchName", label: "Jenkins Branch Name", value: event.pipeline.jenkinsBranchName ?? "" },
    { key: "jenkinsBranchIsPrimary", label: "Jenkins Primary Branch", value: event.pipeline.jenkinsBranchIsPrimary !== undefined ? String(event.pipeline.jenkinsBranchIsPrimary) : "" },
    { key: "jenkinsChangeId", label: "Jenkins Change ID (PR)", value: event.pipeline.jenkinsChangeId ?? "" },
    { key: "jenkinsChangeUrl", label: "Jenkins Change URL", value: event.pipeline.jenkinsChangeUrl ?? "" },
    { key: "jenkinsChangeTitle", label: "Jenkins Change Title", value: event.pipeline.jenkinsChangeTitle ?? "" },
    { key: "jenkinsChangeAuthor", label: "Jenkins Change Author", value: event.pipeline.jenkinsChangeAuthor ?? "" },
    { key: "jenkinsChangeAuthorDisplayName", label: "Jenkins Change Author Display Name", value: event.pipeline.jenkinsChangeAuthorDisplayName ?? "" },
    { key: "jenkinsChangeAuthorEmail", label: "Jenkins Change Author Email", value: event.pipeline.jenkinsChangeAuthorEmail ?? "" },
    { key: "jenkinsChangeTarget", label: "Jenkins Change Target", value: event.pipeline.jenkinsChangeTarget ?? "" },
    { key: "jenkinsChangeBranch", label: "Jenkins Change Branch", value: event.pipeline.jenkinsChangeBranch ?? "" },
    { key: "jenkinsChangeFork", label: "Jenkins Change Fork", value: event.pipeline.jenkinsChangeFork ?? "" },
    { key: "jenkinsTagName", label: "Jenkins Tag Name", value: event.pipeline.jenkinsTagName ?? "" },
    { key: "jenkinsGitCommit", label: "Jenkins Git Commit", value: event.pipeline.jenkinsGitCommit ?? "" },
    { key: "jenkinsGitPreviousCommit", label: "Jenkins Git Prev Commit", value: event.pipeline.jenkinsGitPreviousCommit ?? "" },
    { key: "jenkinsGitPreviousSuccessfulCommit", label: "Jenkins Git Prev Successful Commit", value: event.pipeline.jenkinsGitPreviousSuccessfulCommit ?? "" },
    { key: "jenkinsGitBranch", label: "Jenkins Git Branch", value: event.pipeline.jenkinsGitBranch ?? "" },
    { key: "jenkinsGitLocalBranch", label: "Jenkins Git Local Branch", value: event.pipeline.jenkinsGitLocalBranch ?? "" },
    { key: "jenkinsGitUrl", label: "Jenkins Git URL", value: event.pipeline.jenkinsGitUrl ?? "" },
    { key: "jenkinsGitAuthorName", label: "Jenkins Git Author Name", value: event.pipeline.jenkinsGitAuthorName ?? "" },
    { key: "jenkinsGitAuthorEmail", label: "Jenkins Git Author Email", value: event.pipeline.jenkinsGitAuthorEmail ?? "" },
    { key: "jenkinsGitCommitterName", label: "Jenkins Git Committer Name", value: event.pipeline.jenkinsGitCommitterName ?? "" },
    { key: "jenkinsGitCommitterEmail", label: "Jenkins Git Committer Email", value: event.pipeline.jenkinsGitCommitterEmail ?? "" },
    { key: "jenkinsRunDisplayUrl", label: "Jenkins Run Display URL", value: event.pipeline.jenkinsRunDisplayUrl ?? "" },
    { key: "jenkinsJobDisplayUrl", label: "Jenkins Job Display URL", value: event.pipeline.jenkinsJobDisplayUrl ?? "" },
    { key: "jenkinsBuildCause", label: "Jenkins Build Cause", value: event.pipeline.jenkinsBuildCause ?? "" },
    { key: "jenkinsRootBuildCause", label: "Jenkins Root Build Cause", value: event.pipeline.jenkinsRootBuildCause ?? "" },
    { key: "jenkinsBuildUser", label: "Jenkins Build User", value: event.pipeline.jenkinsBuildUser ?? "" },
    { key: "jenkinsBuildUserId", label: "Jenkins Build User ID", value: event.pipeline.jenkinsBuildUserId ?? "" },
    { key: "jenkinsBuildUserEmail", label: "Jenkins Build User Email", value: event.pipeline.jenkinsBuildUserEmail ?? "" },
    { key: "jenkinsStartedBy", label: "Jenkins Started By", value: event.pipeline.jenkinsStartedBy ?? "" },
    { key: "jenkinsStageName", label: "Jenkins Stage Name", value: event.pipeline.jenkinsStageName ?? "" },
    {
      key: "duration",
      label: "Duration",
      value: event.durationMs ? `${(event.durationMs / 1000).toFixed(1)}s` : "",
    },
    {
      key: "startedAt",
      label: "Started At",
      value: event.startedAt ? new Date(event.startedAt).toUTCString() : "",
    },
  ];

  // Define which fields are considered "Core" (Always show if non-empty)
  const CORE_FIELDS = new Set([
    "pipeline",
    "repository",
    "branch",
    "commit",
    "commitmessage",
    "step",
    "environment",
    "source",
    "triggeredby",
  ]);

  // Add custom user-provided metadata to the pool of available fields
  if (event.metadata) {
    for (const [key, value] of Object.entries(event.metadata)) {
      const displayKey = key.charAt(0).toUpperCase() + key.slice(1).replace(/[_-]/g, " ");
      allFields.push({ key, label: displayKey, value });
    }
  }

  // Add dynamic release artifacts if they exist
  if (event.pipeline.releaseArtifacts) {
    for (const [key, value] of Object.entries(event.pipeline.releaseArtifacts)) {
      const displayKey = key.replace(/^Release\.Artifacts\./, "Artifact: ");
      allFields.push({ key, label: displayKey, value: String(value) });
    }
  }

  // Determine which fields to display
  let fieldsToDisplay = [];

  if (displayMetadata && displayMetadata.length > 0) {
    // Whitelist mode: show ONLY what the user explicitly asked for in displayMetadata
    // Resolve aliases (e.g. repo -> repository, commit_id -> commit, run_number -> runNumber)
    const whitelist = new Set(
      displayMetadata.map((raw) => {
        const cleaned = raw.trim().toLowerCase();
        const resolved = METADATA_FIELD_ALIASES[cleaned] || cleaned;
        return resolved.toLowerCase();
      })
    );
    fieldsToDisplay = allFields.filter((f) => whitelist.has(f.key.toLowerCase()));
  } else {
    // Smart Default mode:
    // 1. Show Core fields (if they have a value)
    // 2. Show fields the user explicitly passed as CLI flags
    // 3. Show any custom metadata passed via --meta
    const explicitSet = new Set((event.explicitFields || []).map((f) => f.toLowerCase()));

    fieldsToDisplay = allFields.filter((f) => {
      if (f.value === "") return false;

      const isCore = CORE_FIELDS.has(f.key.toLowerCase());
      const isExplicit = explicitSet.has(f.key.toLowerCase());
      const isCustomMeta = event.metadata && event.metadata[f.key] !== undefined;

      return isCore || isExplicit || isCustomMeta;
    });
  }

  if (fieldsToDisplay.length > 0) {
    out.push("## Pipeline Metadata");
    out.push("| Field | Value |");
    out.push("| --- | --- |");
    for (const field of fieldsToDisplay) {
      out.push(`| ${field.label} | ${field.value} |`);
    }
    out.push("");
  }

  if (event.failure.errorMessage) {
    out.push("### Error Message");
    out.push("```");
    out.push(event.failure.errorMessage);
    out.push("```");
    out.push("");
  }

  // Log excerpt — step-aware smart excerpt + secret-masked
  if (event.failure.logs) {
    const cleaned = maskLogs ? maskSecrets(event.failure.logs) : event.failure.logs;
    const { text, failingStep } = buildSmartExcerpt(cleaned, event.source, logExcerptLines);
    const logHeader = failingStep ? `### Failing Step: ${failingStep}` : "### Relevant Logs";
    out.push(logHeader);
    if (event.failure.logsTruncated) {
      out.push("> Logs were truncated by the adapter — see attachment for full output.");
    }
    out.push("```log");
    out.push(text);
    out.push("```");
    out.push("");
  }

  // Links
  const links: string[] = [];
  if (event.pipeline.url) {
    links.push(`- [Pipeline](${event.pipeline.url})`);
  }
  if (event.pipeline.runUrl) {
    links.push(`- [Pipeline Run](${event.pipeline.runUrl})`);
  }
  if (event.repository.url) {
    links.push(`- [Repository](${event.repository.url})`);
  }
  if (event.commit.url) {
    links.push(`- [Commit](${event.commit.url})`);
  }
  if (event.pullRequest?.url) {
    links.push(`- [Pull Request #${event.pullRequest.number}](${event.pullRequest.url})`);
  }
  // Additional external links from fields
  const externalLinks = (fields.externalLinks as any[]) || [];
  const renderedTitles = new Set(["Pipeline", "Pipeline Run", "Repository", "Commit"]);
  for (const link of externalLinks) {
    if (
      renderedTitles.has(link.title) ||
      link.title.startsWith("Commit ") ||
      link.title.startsWith("PR #") ||
      link.title.startsWith("Pull Request #")
    ) {
      continue;
    }
    if (link.url) {
      links.push(`- [${link.title}](${link.url})`);
    }
  }

  if (links.length > 0) {
    out.push("### Links");
    out.push(...links);
    out.push("");
  }

  // Provenance footer — small, builds trust about which stage produced what.
  if (fields.provenance && Object.keys(fields.provenance).length > 0) {
    out.push("---");
    out.push("");
    out.push(
      `<sub>Generated by PipelineIQ · signature \`${fields.dedupSignature ?? "?"}\`</sub>`,
    );
  }

  return out.join("\n");
}
