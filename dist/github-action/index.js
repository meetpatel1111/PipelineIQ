...
async function run() {
  try {
    ...
    const ghCtx = {
      ...
      runAttempt: core.getInput("github-run-attempt") || process.env.GITHUB_RUN_ATTEMPT ? Number.parseInt(core.getInput("github-run-attempt") || process.env.GITHUB_RUN_ATTEMPT, 10) : void 0,
      ...
      visibility: github.context.payload.repository?.visibility,
      // Runner information
      ...
      strategyJobIndex: core.getInput("strategy-job-index") ? Number.parseInt(core.getInput("strategy-job-index"), 10) : void 0,
      strategyJobTotal: core.getInput("strategy-job-total") ? Number.parseInt(core.getInput("strategy-job-total"), 10) : void 0,
      ...
      retentionDays: core.getInput("github-retention-days") || process.env.GITHUB_RETENTION_DAYS ? Number.parseInt(core.getInput("github-retention-days") || process.env.GITHUB_RETENTION_DAYS, 10) : void 0
    };
    ...
    if (result.selfHealing) {
      ...
      if (result.selfHealing.fix) {
        ...
        try {
          let summary2 = core.summary.addHeading("\u{1F916} PipelineIQ Self-Healing Fix Summary", 2);
          if (result.selfHealing.success && result.selfHealing.prUrl) {
            summary2 = summary2.addRaw(`\u2705 **PR Created:** [${result.selfHealing.prUrl}](${result.selfHealing.prUrl})<br>`);
            summary2 = summary2.addRaw(`\u{1F33F} **Branch:** \`${result.selfHealing.branchName}\`<br>`);
          } else if (result.selfHealing.dryRun) {
            summary2 = summary2.addRaw(`\u{1F9EA} **Dry Run Mode \u2014 No PR created**<br>`);
          } else {
            summary2 = summary2.addRaw(`\u274C **Self-Healing Failed:** ${result.selfHealing.reason}<br>`);
          }
          summary2 = summary2.addRaw(`\u{1F3AF} **Confidence:** ${Math.round(fix.confidence * 100)}% | \u26A1 **Risk Level:** ${fix.riskLevel}<br><br>`).addHeading("Proposed File Changes", 3);
          const tableHeaders = [
          ...
          await summary2.addTable([tableHeaders, ...tableRows]).write();
        } catch (sumErr) {
...
function readConfig() {
  const aiMode = core.getInput("ai-mode") || "disabled";
  const raw = {
    jira: {
      type: core.getInput("jira-type") || "cloud",
      ...
      username: core.getInput("jira-username") || void 0,
      password: core.getInput("jira-password") || void 0,
    ...
    ...core.getInput("default-assignee") ? { defaultAssignee: core.getInput("default-assignee") } : {},
    ai: {
      mode: aiMode,
      ...core.getInput("ai-provider") ? { provider: core.getInput("ai-provider") } : {},
