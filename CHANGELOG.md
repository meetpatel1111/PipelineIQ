...
### Added
- **Native Bitbucket Cloud Pipelines Support**: Automatic, zero-config ingestion for Bitbucket Cloud builds. Seamlessly captures repository context, workspace details, parallel steps, deployments, and merge queues directly into Jira tickets.
- **Comprehensive CLI Flag Overrides**: Every operational metadata field can now be explicitly set or overridden via CLI flags (including `--step`, `--stage`, `--exit-code`, `--error-message`, `--pr-number`, `--pr-title`, `--runner-id`, and `--environment-tier`), ensuring CLI options always take precedence over ambient runner variables.
- **Bitbucket Pipelines Runner Log Parser**: High-performance parser that strips terminal ANSI colors, normalizes carriage returns, maps executed step commands (`+ <cmd>`), and extracts error diagnostics and exit codes.
- **Bitbucket Secret Masking Firewall**: Automatically detects and redacts Bitbucket Personal Access Tokens, Workspace Access Tokens, and Repository Access Tokens before logs or diagnostics reach Jira.
- **Deep Atlassian Ecosystem Linking**: Automatically binds Bitbucket Cloud pipeline runs, pull requests, and commit diffs into Jira Development panels and Remote Web Links.
...
### Added
- **Native GitLab CI/CD Support**: Automatic detection and context ingestion for GitLab CI runners. Captures merge request details, job stages, environments, and runner metadata into Jira incident tickets without requiring custom pipeline plugins.
- **GitLab Runner Log Parser**: Intelligent log parser that strips GitLab terminal section markers (`section_start`/`section_end`), normalizes timestamps, and extracts step failure diagnostics.
- **GitLab Token Secret Masking**: Proactively detects and redacts GitLab Personal Access Tokens, Job Tokens, Deploy Tokens, and Pipeline Trigger Tokens from all tickets and comments.
- **Interactive GitLab Incident Diagnostics**: Populates Jira Development panels with direct links to GitLab merge requests, pipeline runs, and commit diffs.
...
### Added
- **Gemini API Resilience**: Implemented exponential backoff retry logic (up to 3 retries starting at 1000ms delay) in the Gemini provider to gracefully handle temporary `503 Service Unavailable` spikes and rate limits.

### Changed
- **AI Providers Hardening**: Added support for a raw generation mode (`isRawPrompt`) across OpenAI, Anthropic, Gemini, Azure OpenAI, and Local AI providers to bypass standard diagnostic system prompts when performing self-healing. This eliminates the JSON-schema conflict and prevents parsing errors.
